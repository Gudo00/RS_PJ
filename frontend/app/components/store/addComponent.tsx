...
function StoreAddComponent() {
    ...
    const addMutation = useMutation({
        mutationFn: testStoreAddForm,
        onSuccess: (data) => {
            query.invalidateQueries({queryKey: ['stores']});
        }
    });
    ...
    return (
        <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-lg rounded-xl">
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label htmlFor="file" className="block text-sm font-medium text-gray-600 mb-1">
                        첨부파일
                    </label>
                    <input
                        name="file"
            ...
            {addMutation.data && <ResultComponent msg={'등록완료'} closeFn={() => navigate("/store/list")} />}
        </div>
    );
}
